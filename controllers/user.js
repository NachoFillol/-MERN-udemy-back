...
function signUp(req, res) {
    ...
    if(!password || !repeatPassword) {
        res.status(404).send({message: 'Las passwords son obligtorias.'});
    } else {
        if(password !== repeatPassword) {
            res.status(404).send({message: 'Las passwords tienen que ser iguales'});
        } else {
            ...
            bcrypt.hash(password, null, null, function(err, hash) {
                if (err) {
                    ...
                    user.save((err, userStored) => {
                        if(err) {
                            res.status(500).send({message: 'Error usuario ya existe'});
                        } else {
                            if(!userStored) {
            ...
            if(!userData) {
                ...
                if(req.files) {
                    ...
                    if (fileExt !== 'png' && fileExt !== 'jpg') {
                        res.status(400).send({message:'La extension es invalida. (Extensiones validas: .png and .jpg)'});
                    } else {
...
async function updateUser(req, res) {
    ...
    User.findByIdAndUpdate({ _id: params.id }, userData, (err, userUpdate) => {
        if(err) {
            ...
            if(!userUpdate) {
                res.status(404).send({ message: 'No se ha encotnrado ningun Usuario' });
            } else {
